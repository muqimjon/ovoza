...
const CONTENT: Record<Locale, { eyebrow: string; title: string; accent: string; subtitle: string; cta: string; services: Service[] }> = {
  uz: {
    ...
    services: [
      ...
      { icon: 'phone', name: 'IP-telefoniya', title: 'IP-telefoniya', desc: 'Qo‘ng‘iroqlar yozuvi mijoz kartasiga ulanadi, hech narsa yo‘qolmaydi.', features: ['Qo‘ng‘iroqlar yozuvi', 'Mijoz kartasiga ulanish', 'Qo‘ng‘iroq statistikasi', 'Yo‘qolgan qo‘ng‘iroqlar nazorati'] },
  ...
  ru: {
    ...
    services: [
      ...
      { icon: 'phone', name: 'IP-телефония', title: 'IP-телефония', desc: 'Записи звонков привязаны к карточке клиента, ничего не теряется.', features: ['Запись звонков', 'Привязка к карточке клиента', 'Статистика звонков', 'Контроль пропущенных'] },
  ...
  en: {
    ...
    services: [
      ...
      { icon: 'phone', name: 'IP telephony', title: 'IP telephony', desc: 'Call recordings link to the client card — nothing is lost.', features: ['Call recording', 'Linked to client card', 'Call statistics', 'Missed-call control'] },
